...
```
:app
 ├── :feature:auth      ← implemented (LoginScreen, JWT session gating)
...
- **Ktor 3.x removed `URLBuilder.encodedPath`** as a String property. Use `url.encodedPathSegments.joinToString("/")` instead, or work with the segments list directly. In MockEngine test lambdas `request.url` is `Url` (not `URLBuilder`) — use `request.url.encodedPath` (String) there instead.
- **Token storage uses Android Keystore + DataStore, not deprecated `security-crypto`.** `EncryptedSharedPreferences` / `EncryptedFile` from `androidx.security:security-crypto` are deprecated (stuck at 1.1.0-alpha, never stabilised). The project uses `CryptoManager` (AES-GCM, key in Android Keystore, IV+ciphertext stored as Base64) + `EncryptedTokenStore` (DataStore). `CryptoManager.encrypt`/`decrypt` require a real Android Keystore so `CryptoManagerTest` must run as an instrumented test (`androidTest`), not a unit test.
- **`AuthTokenProvider` was replaced by `TokenStore`.** The old `AuthTokenProvider` interface and `StubAuthTokenProvider` are deleted. All tests and DI use `TokenStore` / `StubTokenStore` (in-memory stub). `createPhotoVaultHttpClient` now accepts `tokenStore: TokenStore` (and a `baseUrl: String` overload for tests) instead of `baseUrlProvider + tokenProvider`.
