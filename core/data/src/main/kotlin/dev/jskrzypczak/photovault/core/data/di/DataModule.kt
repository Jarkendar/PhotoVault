...
val dataModule = module {
    // Application-scoped coroutine scope used to keep the DataStore flow alive.
    single<CoroutineScope> { CoroutineScope(SupervisorJob() + Dispatchers.IO) }

    // Lambda breaks the circular dep: HttpClient → BaseUrlProvider → ServerSettingsRepositoryImpl → HttpClient.
    // The lambda is captured at creation time; HttpClient is resolved only when checkHealth() is called.
